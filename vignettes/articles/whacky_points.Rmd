...
d <- whacky.trail |> select(-whacks)
trip(d)
sp::coordinates(d) <- ~lon+lat
sp::proj4string(d) <- sp::CRS("+proj=longlat +datum=WGS84", doCheckCRSArgs = FALSE)
tr <- trip(d, c("time", "id"))
whacky.trail$whacky <- !speedfilter(tr, max.speed = ramb::rb_kn2ms(20) / 1000 * 60 * 60)
whacky.trail
whacky.trail |> rb_mapdeck(col = "whacky")
whacky.trail |> 
  count(whacks, whacky)

whacky.trail$whacky2 <- !sda(tr, smax = ramb::rb_kn2ms(20) / 1000 * 60 * 60, distlim = c(0.001, 1))
whacky.trail
```
