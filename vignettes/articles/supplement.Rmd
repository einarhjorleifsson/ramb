...
The objective here is to provide a more structured code than provided in the supplement, relying fully on tidyverse lingo, including usage of the map-function where possible rather than loops.

The packages needed are all supplied upfront:
...
```{r}
lh_change <- function(x) {
  x <- ifelse(x != lag(x), 1, 0)
  x <- replace_na(x, 1)
  x <- cumsum(x)
  return(x)
}
...
creel.change <- 
  creel %>% 
  group_by(id) %>% 
  mutate(change = lh_change(behaviour)) %>% 
...
m2 <-
  creel %>%
  group_by(id) %>%
  nest(data = c(x, y, rowid, time, lon, lat, behaviour)) %>%
  mutate(data = map(data, as.data.frame)) %>%
  mutate(prepdat = map(data, function(x) moveHMM::prepData(x, type = "UTM", coordNames = c("x", "y")))) %>%
  mutate(prepdat = map(prepdat, filter, !is.na(step))) %>%
  mutate(prepdat = map(prepdat, mutate, speed = step/60 * 1.9438)) %>%
  mutate(m2 = map(prepdat, function(x) mixtools::normalmixEM(x$speed, mu = c(1,4,8), sigma = c(1,1,1)))) %>%
  mutate(threshold = map_dbl(m2, function(x) x$mu[1] + 1.96 * x$sigma[1]))
d <-
  m2 %>%
  unnest(prepdat) %>%
  ungroup() %>%
  mutate(Fishing = ifelse(speed <= threshold, TRUE, FALSE))
...
m3 <-
  creel %>%
  select(id:time, lon:behaviour) %>%
  nest(data = c(time, lon, lat, behaviour, rowid)) %>%
  # function EMbC expects a data.frame, a tibble is a no, no
  mutate(data = map(data, as.data.frame)) %>%
  mutate(m3 = map(data, EMbC::stbc, info = -1)) %>%
  mutate(m3.tbl = map(m3, ramb::tidy_bin_clst_path))
d <-
  m3 %>%
  unnest(m3.tbl) %>%
  mutate(speed = speed * 1.9438,
         # hauling events are associated with low speeds, low turning angles
         # EINAR: Note that 2 has high turning angle
         Fishing = ifelse(A %in% 1:2, TRUE, FALSE))
d %>%
  count(behaviour, A) %>%
  spread(A, n) %>%
  knitr::kable()
...
m4 <-
  ...
  nest(data = c(time, x, y, lon, lat, behaviour)) %>%
  # function prepData expects a data.frame, a tibble is a no, no
  mutate(data = map(data, as.data.frame)) %>%
  mutate(data2 = map(data, moveHMM::prepData, type = "UTM", coordNames = c("x", "y"))) %>%
  # get rid of NA's
  mutate(data2 = map(data2, filter, !is.na(step))) %>%
  # get starting values for the parameters
  mutate(mixEM = map(data2, function(x) mixtools::normalmixEM(x$step, mu = c(20,150,300), sigma =c(20,20,20)))) %>%
  # extract the parameters - note, should possibly be just one step, all in a single function
  mutate(par0 = map(mixEM, par_mixEM)) %>%
  mutate(m4 = map2(data2, par0, lh_fitHMM)) %>%
  mutate(ks4 = map(m4, tidy_movehmm)) %>%
  mutate(vit4 = map(m4, moveHMM::viterbi))
d <- 
  m4 %>%
  select(id, data2, vit4) %>%
  unnest(c(data2, vit4))
d %>%
  count(behaviour, vit4) %>%
  spread(vit4, n) %>%
  knitr::kable()
p.base +
  geom_point(data = d,
             aes(time, step, colour = factor(vit4)),
             size = 0.5) +
  scale_colour_brewer(palette = "Set1") +
...
d %>%
  ...
  ggplot(aes(time, step, colour = factor(vit4))) +
  geom_point() +
...
m5 <-
  ...
  # extract the parameters - note, should possibly be just one step, all in a single function
  mutate(par0 = map(mixEM, par_mixEM)) %>%
  mutate(m5 = map2(data2, par0, lh_fitHMM_model5)) %>%
  mutate(ks5 = map(m5, tidy_movehmm)) %>%
  mutate(vit5 = map(m5, moveHMM::viterbi))
d <- 
  m5 %>%
  select(id, data2, vit5) %>%
  unnest(c(data2, vit5))
d %>%
  count(behaviour, vit5) %>%
  spread(vit5, n) %>%
  knitr::kable()
p.base +
  geom_point(data = d,
             aes(time, step, colour = factor(vit5)),
             size = 0.5) +
  scale_colour_brewer(palette = "Set1") +
  labs(x = "Time", y = "Step [m]") +
  theme(legend.position = c(0.8, 0.2))
d %>% 
  mutate(id = paste(id, behaviour)) %>%
  ggplot(aes(time, step, colour = factor(vit5))) +
  geom_point() +
  facet_wrap(~id, scales = "free", ncol = 3) +
  scale_colour_brewer(palette = "Set1")
```
...
## The supplement, including the output

Only minor changes have been made to the original document. Here output of the data is also provided.
