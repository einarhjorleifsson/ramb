...
dc_spread_cash_and_catch <- function(trails, events, remove_diagnostic = TRUE) {
  # Input checks ----
  required_trails <- c(".pid", "VE_COU", "VE_REF", "SI_DATIM", "FT_REF", "LE_RECT", "SI_STATE")
  required_events <- c("VE_COU", "VE_REF", "FT_REF", "LE_CDAT", "LE_RECT", "LE_KG", "LE_EURO")
  ...
  if (!inherits(trails$SI_DATIM, "POSIXct")) stop("SI_DATIM in trails must be POSIXct/datetime")
  if (!is.character(trails$FT_REF)) stop("FT_REF in trails must be character")
  if (!is.character(trails$LE_RECT)) stop("LE_RECT in trails must be character")
  if (!is.integer(trails$SI_STATE)) stop("SI_STATE in trails must be integer")
  if (!all(trails$SI_STATE %in% c(0,1))) stop("SI_STATE in trails must be 0 or 1")
  ...
  if (!inherits(events$LE_CDAT, "Date")) stop("LE_CDAT in events must be Date")
  ...
  trails <- trails |>
    dplyr::mutate(.year = lubridate::year(SI_DATIM),
                  .yday = lubridate::yday(SI_DATIM))
  #.pid = dplyr::row_number())
  events <- events |>
    dplyr::mutate(.year = lubridate::year(LE_CDAT),
                  .yday = lubridate::yday(LE_CDAT),
  ...
  trails1 <- trails |>
    dplyr::inner_join(events,
                      by = c("VE_COU", "VE_REF", "FT_REF", "LE_RECT", ".year", ".yday", "SI_STATE")) |>
    dplyr::group_by(VE_COU, VE_REF, FT_REF, LE_RECT, .year, .yday, .eid) |>
    dplyr::mutate(
      .wt = ifelse(SI_STATE == 1, 1 / dplyr::n(), NA_real_),
      LE_KG = LE_KG * .wt,
      LE_EURO = LE_EURO * .wt,
  ...
  PID <- trails1$.pid
  EID <- unique(trails1$.eid)
  
  # Step 2: Join by trip+rectangle+year for unmatched ----
  print("Step2")
  trails_left <- dplyr::anti_join(trails, tibble::tibble(.pid = PID), by = ".pid")
  events_left <- dplyr::anti_join(events, tibble::tibble(.eid = EID), by = ".eid")
  
  trails2 <- 
    trails_left |>
    dplyr::select(-.yday) |>
    dplyr::inner_join(events_left |> dplyr::select(-.yday), 
                      by = c("VE_COU", "VE_REF", "FT_REF", "LE_RECT", ".year", "SI_STATE")) |>
    dplyr::group_by(VE_COU, VE_REF, FT_REF, LE_RECT, .year, .eid, SI_STATE) |>
    dplyr::mutate(
      .wt = ifelse(SI_STATE == 1, 1 / dplyr::n(), NA_real_),
      LE_KG = LE_KG * .wt,
      LE_EURO = LE_EURO * .wt,
  ...
  PID <- c(PID, trails2$.pid)
  EID <- unique(c(EID, trails2$.eid))
  
  # Step 3: Join by trip+year for unmatched, allocate only remainder ----
  #         This may fail if SI_STATE remaining are all zero
  trails_left <- dplyr::anti_join(trails, tibble::tibble(.pid = PID), by = ".pid")
  events_left <- 
    dplyr::anti_join(events, tibble::tibble(.eid = EID), by = ".eid") |> 
    dplyr::group_by(VE_COU, VE_REF, FT_REF, .year, SI_STATE) |> 
    dplyr::summarise(LE_KG = sum(LE_KG),
                     LE_EURO = sum(LE_EURO),
                     .groups = "drop")
  trails3 <- trails_left |>
    dplyr::select(-c(.yday, LE_RECT)) |>
    dplyr::left_join(events_left, 
                     by = c("VE_COU", "VE_REF", "FT_REF", ".year", "SI_STATE")) |>
    dplyr::group_by(VE_COU, VE_REF, FT_REF, .year, SI_STATE) |>
    dplyr::mutate(
      .wt = ifelse(SI_STATE == 1, 1 / dplyr::n(), NA_real_),
      LE_KG = LE_KG * .wt,
  ...
  out <- 
    dplyr::bind_rows(trails1, trails2, trails3) |> 
    dplyr::arrange(.pid)
  ...
  if(remove_diagnostic) {
    out <- 
      out |> 
      dplyr::select(-c(.year, .yday, LE_CDAT, .eid, .wt))
  }
